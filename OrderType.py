...
def OrderTypePage(landingPage, orderTypePage, menuPage): #(unsa e close, unsa e open, unsa e next sa button)
    ...
    # First button
    startButton = Button(buttonFrame, image=DineIn, bg=yellowPalette, borderwidth=0, highlightthickness=0, command=lambda: MenuPage(orderTypePage, menuPage))
    startButton.pack(side=LEFT, padx=20)   # spacing between buttons

    # Second button
    optionButton = Button(buttonFrame, image=TakeOut, bg=yellowPalette, borderwidth=0, highlightthickness=0, command=lambda: MenuPage(orderTypePage, menuPage))
